...
export default function MediaForm({
  ...
  const submitMedia = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log("normal > ", formData.get("rating"));
    console.log("number > ", Number(formData.get("rating")));
    const mediaData = {
  ...
  const generatePrompt = async (targetInput: string) => {
    ...
    try {
      switch (targetInput) {
        ...
        case "plot": {
          const prompt = `Write the Synopsis of the ${category} titled "${titleInputValue}". Do not add topics and summarize it in just a paragraph. If you cannot find a synopsis just say so.`;
          const response = await generateContent(prompt);
          setPromptLoading(false);
          setPlot(response);
          break;
